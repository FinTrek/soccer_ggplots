...
```{r}
aitor_desc <- "Aitor Fernandez has been having a stellar season preventing a otherwise woeful Levante team, who have the worst xGA & xG Difference in the league, from having to fight a relegation battle. Aitor is putting up 0.4 GSAA per 90 (1st) & +6.8 GSAA % (2nd), all the while being the busiest keeper in the league, forced to make a save every 20.3 minutes! He has also prevented 10.4 goals that the post-shot model expected an average GK to concede."

simon_desc <- "7.4 GSAA % best in the league, 5.2 GSAA (3rd best), 3rd best GA per 90 at 0.83, 72.9 Save % (4th best), 0.22 GSAA per 90 (2nd best)
22 GA (3rd best behind Atleti and Real Madrid)
If his teammates further forward could actually score goals the Basques would be in contention for an European spot.
"

oblak_desc <- "The winner of the last 3 Zamora Trophies is having a slight down season, but only in comparison to his God-like form in the past few years. Oblak still has the 2nd best GA per 90 at 0.74 & 3rd best Save % of 73.6 in the league but he has given up 2.7 goals on shots that the post-shot model expected an average GK to save (5th worst)."
```
...
laliga_goalkeeping_MD26_plot <- laliga_gk %>% 
  ...
  geom_mark_hull(
    aes(filter = player == "U. Simón",
        ...
        description = alisson_desc), #position = position_nudge(x = 2),
    expand = unit(2.5, "mm"), con.cap = unit(0, "mm"),
    con.colour = "#228B22", color = "#228B22", 
    label.width = unit(100, 'mm'), label.buffer = unit(1, "mm"),
    label.family = "Roboto Condensed", label.fontsize = c(14, 12),
    label.colour = "grey20", label.fill = "#cce5cc") +
  #annotate("label", x = 0.1, y = 45, fill = "#cce5cc", label = alisson_desc) +
  # J. Oblak
  geom_mark_hull(
    aes(filter = player == "J. Oblak",
        label = "Jan Oblak (Atletico Madrid)",

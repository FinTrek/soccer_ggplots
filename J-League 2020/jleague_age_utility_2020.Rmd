...
create_age_utility_plot <- function(df, color_1, color_2) {
  ...
  plot <- df %>% 
    ...
    scale_y_continuous(
      ...
      labels = percent_format()) +
    scale_x_continuous(
      limits = c(16, max(df$age) + 2),
      breaks = seq(18, max(df$age), by = 2)) +
    labs(
      x = "年齢", 
      y = tategaki_alt("出場時間 (%)"),  
...
create_summary_stats_plot <- function(df) {
  ...
  summary_stats <- summary_df %>%
    ...
    annotate("text", x = 25, y = 0.65, 
             size = 4,
             label = glue::glue("Median Age: {summary_df$median_age}"),
             family = "Yu Mincho Demibold") +
    annotate("text", x = 25, y = 0.55, 
             size = 4,
             label = glue::glue("Median Length of Stay: {summary_df$median_duration} Years"),
             family = "Yu Mincho Demibold") +
    theme_void()

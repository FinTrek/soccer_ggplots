...
```{r}
epl_squad_stats_MD21_raw <- read_csv(here::here("data/epl_squad_stats_MD21.csv"),
                                 skip = 1, col_names = TRUE)

epl_squad_stats_MD21 <- epl_squad_stats_MD21_raw %>% 
  rename_at(vars(12:16), 
            ~ glue::glue("{colnames(epl_squad_stats_MD21_raw)[12:16]}_p90")) %>% 
  rename_at(vars(20:24), 
            ~ glue::glue("{colnames(epl_squad_stats_MD21_raw)[20:24]}_p90"))

epl_squad_stats_MD21 <- epl_squad_stats_MD21 %>% 
  rename_at(vars(2:24), ~ glue::glue("{colnames(epl_squad_stats_MD21)[2:24]}_squad")) %>% 
  rename_at(vars(contains("_1")), ~ str_replace(., "_1", ""))

glimpse(epl_squad_stats_MD21)
```

```{r}
## save
saveRDS(epl_squad_stats_MD21, file = glue("{here::here()}/data/epl_squad_stats_MD21.RDS"))
epl_squad_stats_MD21 <- readRDS(file = glue("{here::here()}/data/epl_squad_stats_MD21.RDS"))
```
...
```{r}
epl_player_stats_MD21_raw <- read_csv(here::here("data/epl_player_stats_MD21.csv"),
                                  skip = 1, col_names = TRUE)

epl_player_stats_MD21 <- epl_player_stats_MD21_raw %>% 
  rename_at(vars(17:21), 
            ~ glue::glue("{colnames(epl_player_stats_MD21_raw)[17:21]}_p90")) %>% 
  rename_at(vars(25:29), 
            ~ glue::glue("{colnames(epl_player_stats_MD21_raw)[25:29]}_p90"))

epl_player_stats_MD21 <- epl_player_stats_MD21 %>% 
  rename_at(vars(contains("_1")), ~ str_replace(., "_1", "")) %>% 
  select(-Matches, -Rk)
  

glimpse(epl_player_stats_MD21)
```

```{r}
## save
saveRDS(epl_player_stats_MD21, file = glue("{here::here()}/data/epl_player_stats_MD21.RDS"))
epl_player_stats_MD21 <- readRDS(file = glue("{here::here()}/data/epl_player_stats_MD21.RDS"))
```
...
```{r}
goal_contribution_clean_df <- epl_player_stats_MD21 %>% 
  left_join(epl_squad_stats_MD21, by = "Squad") %>% 

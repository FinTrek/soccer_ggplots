...
```{r}
desc_jesus <- "Jesus leads the league with 0.84 np-xG per 90, however, for all the chances that fall his way he has only converted 6 goals from 8.5 xG."
...
desc_mahrez_debruyne <- "Mahrez & De Bruyne create the chances that Sterling, Aguero, and to a lesser extent - Gabriel Jesus convert. The two lead the league in Passes into the Penalty Area (PPA) while De Bruyne racks up 4.02 Key Passes (KP) per 90, the best in the league by far."
```
...
xG_xA_per90_plot <- xG_xA_per90 %>% 
  ...
  geom_point(data = xG_xA_per90 %>%
               filter(xA_p90 < 0.14 | npxG_p90 < 0.29,
                      Min >= 900),
             color = "grey20", size = 4, alpha = 0.2) +
  geom_point(data = xG_xA_per90 %>%
               filter(xA_p90 > 0.14 | npxG_p90 > 0.29,
                      Min >= 900),
             color = "red", size = 4) +
  geom_hline(yintercept = 0.29, color = "grey20", alpha = 0.8) +
  geom_vline(xintercept = 0.14, color = "grey20", alpha = 0.8) +
  geom_text_repel(
    data = xG_xA_per90 %>%
      filter(xA_p90 > 0.14 | npxG_p90 > 0.29, 
  ...
  geom_text_repel(
    data = xG_xA_per90 %>%
      filter(player %in% c("R. Sterling", "S. Agüero",
                           "R. Mahrez", "K. De Bruyne")),
    aes(label = player, family = "Roboto Condensed", fontface = "bold"), 
    seed = 15, size = 4.5, 
    min.segment.length = 0, segment.color = "red",
  ...
  geom_mark_hull(
    ...
    expand = unit(0.1, "mm"), label.width = unit(70, 'mm'),
  ...
  geom_mark_hull(
    ...
    expand = unit(2.22, "mm"), label.width = unit(80, 'mm'),
  ...
  scale_y_continuous(labels = seq(0, 0.9, 0.1), 
                     breaks = seq(0, 0.9, 0.1),
                     limits = c(0, 0.9)) +
  labs(title = "City's Attackers Dominate: ", 
       subtitle = glue("
                       Premier League (2019-2020 Season) | Matchday 20 | December 31st, 2019
                       Average (Median) for Midfielders/Forwards | Minimum 900 Minutes Played
                       Average np-xG per 90: 0.29 | Average xA per 90: 0.14"),
       caption = glue("
                      Data: FBref | StatsBomb
                      Twitter: @R_by_Ryo"),
...
ggsave(plot = xG_xA_per90_plot, 
       here::here("Premier League 2019-2020/output/xA_xG_per90_1920_MD20.png"),
       height = 10, width = 12)
```

```{r}
plot_logo <- add_logo(
  plot_path = here::here("Premier League 2019-2020/output/xA_xG_per90_1920_MD20.png"),
  logo_path = "https://upload.wikimedia.org/wikipedia/en/f/f2/Premier_League_Logo.svg",
  logo_position = "top right",
  logo_scale = 6)
...
```{r}
image_write(image = plot_logo, 
            here::here("Premier League 2019-2020/output/xA_xG_per90_1920_MD20_logo.png"))
```

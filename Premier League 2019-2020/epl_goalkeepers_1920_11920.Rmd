...
```{r}
alisson_desc <- "Alisson proving to be the best keeper this season with his 9.96 GSAA % and 83.3% Save %, 0.41 Goals Against per 90 all leading the Premier League by a considerable margin."

kepa_desc <- "Kepa is the least busiest keeper in the league having to only make a save every 50.49 minutes on average yet he has the 4th worst GSAA in the league having given up 5.5 goals on shots that the post-shot model expected an average GK to save."

gunn_desc <- "Angus Gunn was giving up a league-worst 2.5 Goals Against per 90 and -11.35 GSAA % before he was replaced after the horrendous 0-9 loss to Leicester City. His replacement Alex McCarthy has been doing much better with +2 GSAA % and 1.31 GA per 90!"
```
...
epl_goalkeeping_MD23_plot <- epl_gk %>% 
  ...
  geom_mark_hull(
    ...
    label.width = unit(140, 'mm'), label.buffer = unit(5, "mm"),
    label.family = "Roboto Condensed", label.fontsize = c(14, 12),
  ...
  scale_y_continuous(breaks = seq(20, 56, 10),
                     limits = c(18, 56),
                     expand = c(0.01, 0)) +
  labs(title = "Goalkeeper Performance & Workload: <b style='color: red'>Premier League (2019-2020)</b>",
       subtitle = glue::glue("
                             January 19th, 2020 | MD23 | Minimum 900 Minutes Played
...
ggsave(plot = epl_goalkeeping_MD23_plot, 
       here::here("Bundesliga 2019-2020/output/epl_goalkeeping_1920_MD23.png"),
       height = 10, width = 14)
```

```{r}
plot_logo <- add_logo(
  plot_path = here::here("Bundesliga 2019-2020/output/epl_goalkeeping_1920_MD23.png"),
...
image_write(image = plot_logo, 
            here::here("Bundesliga 2019-2020/output/epl_goalkeeping_1920_MD23_logo.png"))
```

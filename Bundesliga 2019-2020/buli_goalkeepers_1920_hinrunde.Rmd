...
buli_gk <- buli_player_goalkeeping_hinrunde_clean %>% 
  ...
  mutate(fillcol = case_when(
    GSAA_perc < -0.017 & minPerSave < 28.86 ~ "red",
    GSAA_perc > -0.017 & minPerSave < 28.86 ~ "yellow",
    GSAA_perc < -0.017 & minPerSave > 28.86 ~ "orange",
    GSAA_perc > -0.017 & minPerSave > 28.86 ~ "green",
    TRUE ~ NA_character_))
```
...
buli_gk %>% 
  ...
  geom_mark_hull(
    ...
    label.width = unit(50, 'mm'), label.buffer = unit(1, "mm"),
    label.family = "Roboto Condensed", label.fontsize = c(14, 12),
    label.colour = "grey20", label.fill = "#cce5cc") +
  # Sommer
  geom_mark_hull(
    aes(filter = player == "Y. Sommer",
        ...
        description = "brandt_desc"),
    expand = unit(2.5, "mm"), con.cap = unit(0, "mm"),
    label.width = unit(90, 'mm'), label.buffer = unit(75, "mm"),
    label.family = "Roboto Condensed", label.fontsize = c(14, 12),
    label.colour = "grey20", label.fill = "#cce5cc") +
  ...
  scale_y_continuous(breaks = seq(20, 60, 10),
                     limits = c(18, 60),
                     expand = c(0.01, 0)) +
  labs(title = "Goalkeeper Performance vs. Saves",
       subtitle = "How Busy Keepers are compared to How Many Goals They are Conceded Above/Below the Average GK",

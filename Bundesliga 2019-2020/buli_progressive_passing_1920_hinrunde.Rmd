...
```{r}
bayern_desc <- "Both Kimmich & Alaba have been playing much more centrally this season (Center Midfield & Center Back respectively) the former only behind teammate Coutinho and Brandt in Passes into the Penalty Area per 90 with the latter leading the league with 9.83 Passes into the Final 3rd per 90. Thiago keeps things ticking with a 90.2% Pass Accuracy."

brandt_desc <- "Julian Brandt has become the primary ball progressor in this Dortmund side with his 2.99 PPA per 90 leading the league by a considerable margin. A lot of this comes from his throughballs (0.68 TB per 90, 1st in the league). Along with his 30th ranking in both Final Third Passes per 90 & xA per 90 it shows that he provides that incisive ball in between the build-up & the final pass. Indeed, his KP per 90 is 47th in the league."
```
...
buli_progressive_pass_hinrunde_plot <- ggplot(buli_pass_df, 
  ...
  geom_mark_hull(
    ...
    expand = unit(2.5, "mm"), con.cap = unit(0, "mm"),
    label.width = unit(250, 'mm'), label.buffer = unit(1, "mm"),
    label.family = "Roboto Condensed", label.fontsize = c(14, 12),
    label.colour = "grey20", label.fill = "#cce5cc") + 
  ...
  scale_y_continuous(labels = seq(0, 4, 0.5), 
                     ...
                     limits = c(0, 4),
                     expand = c(0.01, 0)) +
  labs(title = "Progressive Passers: <b style='color: red'>Bundesliga</b>", 
       subtitle = glue("
                       Bundesliga (2019-2020) | Hinrunde | January 15th, 2020
...
N. Gießelmann, Kostic, Diego Demme, Florian Neuhaus, Coray Gunter
